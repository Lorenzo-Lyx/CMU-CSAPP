...
char msg[MAXLINE<<1];      /* for whenever we need to compose an error message */
...
static trace_t *read_trace(char *tracedir, char *filename)
{
	...
	int fscanf_unused_value = 0;
    fscanf_unused_value = fscanf(tracefile, "%d", &(trace->sugg_heapsize)); /* not used */
    fscanf_unused_value = fscanf(tracefile, "%d", &(trace->num_ids));     
    fscanf_unused_value = fscanf(tracefile, "%d", &(trace->num_ops));     
    fscanf_unused_value = fscanf(tracefile, "%d", &(trace->weight));        /* not used */
	...
	switch(type[0]) {
	case 'a':
	    fscanf_unused_value = fscanf(tracefile, "%u %u", &index, &size);
	...
	case 'r':
	    ...
	    break;
	case 'f':
	    fscanf_unused_value = fscanf(tracefile, "%ud", &index);
